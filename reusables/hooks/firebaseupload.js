...
const uploadFirebase = async (mp) => {
  ...
  var fileIDTypeChecker =
    !mp.referenceMediaType.includes("audio") &&
    !mp.referenceMediaType.includes("video") &&
  ...
  var fileTypeChecker =
    ...
    !mp.referenceMediaType.includes("image")
      ...
      : folderDesignation[mp.referenceMediaType.split("/")[0]];
  ...
  return await file
    ...
    .then((url) => {
      const publicUrl = mp.referenceMediaType.includes("image")
        ? `https://storage.googleapis.com/${FIREBASE_STORAGE_BUCKET}/${finalPathwithID}`
        : `https://storage.googleapis.com/${FIREBASE_STORAGE_BUCKET}/${finalPathwithIDEncoded}`;
      return publicUrl;
    })
    .catch((err) => {
      console.log(err);
