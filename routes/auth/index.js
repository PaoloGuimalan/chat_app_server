...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    const middleName =
      decodeToken.fullname.middleName &&
      decodeToken.fullname.middleName.trim() != ""
        ? decodeToken.fullname.middleName
        : "N/A";
...
const verifyCodeUserId = async (userIDProp, code) => {
  return await UserVerification.findOne({ userID: userIDProp, verCode: code })
    .then((result) => {
      if (result) {
        if (!result.isUsed) {
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    await UserAccount.findOne({
      $or: [{ userID: emailOrUsername }, { email: emailOrUsername }],
    })
      .then((result) => {
        if (result) {
